<?php
function nahoda() //aby mi fungovalo refreshovanie stranky (pomohol mi s tým učiteľ)
{
    $znaky = "1234567890asdfghjklqwertyuiopzxcvbnm";
    $vystup = "";
    for ($i = 0; $i < 10; $i++) {
        $vystup .= $znaky[rand(0, strlen($znaky) - 1)];
    }
    return $vystup;
}
    require_once '../core/init.php';
    if(!is_logged_in()){
        header("Location: login.php");
    }
    include 'includes/head.php';
    include 'includes/navigation.php';

    if(isset($_GET['complete']) && $_GET['complete'] == 1){
        $cart_id = sanitize((int)$_GET['cart_id']);
        $db->query("UPDATE cart SET shipped = 1 WHERE id = '{$cart_id}'");
        $_SESSION['success_flash'] = "Objednávka bola dokončená!";
        header('Location: index.php');

    }

    $txn_id = sanitize((int)$_GET['txn_id']);
    $txnQuery = $db->query("SELECT * FROM transactions WHERE id = '{$txn_id}'");
    $txn = mysqli_fetch_assoc($txnQuery);
    $cart_id = $txn['cart_id'];
    $cartQ = $db->query("SELECT * FROM cart WHERE id = '{$cart_id}'");
    $cart = mysqli_fetch_assoc($cartQ);
    $items = json_decode($cart['items'], true);
    $idArray = array();
    $produkty = array();
    foreach($items as $item){
        $idArray[] = $item['id']; 
    }
    $ids = implode(',',$idArray);
    $produktQ = $db->query(
        "SELECT i.id as 'id', i.title as 'title', c.id as 'cid', c.category as 'child', p.category as 'parent'
        FROM products i 
        LEFT JOIN categories c ON i.categories = c.id
        LEFT JOIN categories p ON c.parent = p.id
        WHERE i.id IN ({$ids});
    ");
    while($p = mysqli_fetch_assoc($produktQ)){

        foreach($items as $item){
            if($item['id'] == $p['id']){
                $x = $item;
                continue;
            }
        }
        $produkty[] = array_merge($x, $p); 
    }   
?>
    <h2 class="text-center">Objednané produkty</h2>
    <table class="table table-condensed table-bordered table-striped">
        <thead>
            <th>Množstvo</th>
            <th>Názov</th>
            <th>Kategória</th>
        </thead>
        <tbody>
            <?php foreach($produkty as $produkt):?>
                <tr>
                    <td><?=$produkt['quantity'];?></td>
                    <td><?=$produkt['title'];?></td>
                    <td><?=$produkt['parent'].' ~ '.$produkt['child'];?></td>
                </tr>
            <?php endforeach;?>
        </tbody>
    </table>

    <div class="row">
        <div class="col-md-6">
            <h3 class="text-center">Detaily objednávky</h3>
            <table class="table table-condensed table-striped table-bordered">
                <tbody>
                    <tr>
                        <td>Medzisúčet</td>
                        <td><?=money($txn['sub_total']);?></td>
                    </tr>
                    <tr>
                        <td>Daň</td>
                        <td><?=money($txn['tax']);?></td>
                    </tr>
                    <tr>
                        <td>Celková suma</td>
                        <td><?=money($txn['grand_total']);?></td>
                    </tr>
                    <tr>
                        <td>Dátum objednávky</td>
                        <td><?=pretty_date($txn['txn_date'])?></td>
                    </tr>
                </tbody>
            </table>
        </div>
        <div class="col-md-6">
            <h3 class="text-center">Dodacia adresa</h3>
            <address>
                <?=$txn['full_name']?><br>
                <?=$txn['street']?><br>
                <?=$txn['street2'] != ''?$txn['street2'].'<br>':'';?>
                <?=$txn['city'].', '.$txn['state']. ' '.$txn['zip_code']; ?><br>
            </address>
        </div>
    </div>
    <div class="pull-right">
        <a href="index.php" class="btn btn-large btn-default">Zatvoriť</a>
        <a href="orders.php?complete=1&cart_id=<?=$cart_id;?>" class="btn btn-primary btn-large">Dokončiť objednávku</a>
    </div>

<?php include 'includes/footer.php'?>